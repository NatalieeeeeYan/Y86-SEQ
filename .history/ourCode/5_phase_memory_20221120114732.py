...
def memory(cur, memory):
    ...
    if cur.M.icode in [RMMOVL, PUSHL, CALL, MRMOVL]:    # 这些指令需要写入内存
        memory_address = cur.M.valE
    elif cur.M.icode in [POPL, RET]:    # 这些指令需要读取内存
        memory_address = cur.M.valA
    else:
        memory_address = None

    if cur.M.icode in [MRMOVL, POPL, RET]:
        read_flag = True
    if cur.M.icode in [RMMOVL, PUSHL, CALL]:
        write_flag = True
    error_flag = False
    if read_flag:
        try:
