...
class Memory:
    ...
    def write(self, position, value):
        # 写入内存，value是要写入的内容（字符串形式），position是要写入的位置
        if position < 0 or (position + len(value)) >= MEMSIZE:  # 非法访问/溢出
            raise Exception(
                ("Trying to write memory from %d to %d" % (position, position+len(value))))
        # 由于memory以"00"为一个单位，故其写入到memory中的位置为position ~ position + floor() ---?
        self.memory[position: position + len(value) // 2] = split(value, 2)

    def get_instruntion(self, instruction):
        # 指令e.g.: 0x000: 30f20a00000000000000 |   irmovq $10,%rdx
        # ｜之前是指令序列，找到｜提取前方的子串获得指令序列
        # 指令序列前
        for line in instruction:
            if line.find('|') != -1:
                line = line[:line.find('|')]
            if len(line) == 0 or line.isspace():    # 指令只由空格组成，读取下一行
                continue
            position = None
            value = None
            try:
...
if __name__ == "__main__":
    ...
    length = len(value) // 2
    value_ = "hijklmn"
    length_ = len(value_) // 2
    print(myMemory.read(0, length + length_ + 2))
    myMemory.write(0, value)
    for i in range(0, length + 1):
        print(myMemory.memory[i])
    print(myMemory.read(0, length + 1))
    myMemory.write(length + 1, value_)
    for i in range(0, length + length_ + 2):
        print(myMemory.memory[i])
    print(myMemory.read(0, length + length_ + 2))
    myMemory.get_instruntion("0x000: 30f20a00000000000000 |   irmovq $10,%rdx")
