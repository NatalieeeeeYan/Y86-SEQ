...
class Memory:
    ...
    def read(self, position, len):
        if position < 0 or position + len >= MEMSIZE:
            raise Exception(
                ("Trying to read memory from %d to %d" % (position, position+len)))
        return "".join(self.memory[position:position+len])
