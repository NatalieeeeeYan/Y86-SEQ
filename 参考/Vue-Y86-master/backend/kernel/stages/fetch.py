...
def fetch(cur, nxt, mem):
    ...
    if cur.M.icode in [IJXX] and not cur.M.Cnd:
        pc = cur.M.valA
        op.append('Select current PC {0} due to previous JMP misprediction'.format(
            swichEndian(pc)))
    elif cur.W.icode in [IRET]:
        pc = cur.W.valM
        op.append('Select current PC {0} due to previous RET'.format(
            swichEndian(pc)))
    else:
        pc = cur.F.predPC
    ...
    if icode in [IJXX, ICALL]:
        predPC = valC
        op.append('Select next PC {0} due to JMP or CALL'.format(
            swichEndian(predPC)))
    else:
        predPC = valP
